...
import ProtectedPermissionWrap from '../ProtectedPermissionWrap'
  ...
  return (
    <div className="flex flex-col gap-3">
      {navLinks.map((link) => {
        ...
        const isDashboard =
          link.route === '/dashboard' || link.route.startsWith('/dashboard/')

        return (
          <SheetCloseWrapper {...shetCloseWrapperProps} key={link.label}>
            <ProtectedPermissionWrap condition={isDashboard}>
              <Link
                key={link.label}
                href={link.route}
                className={cn(
